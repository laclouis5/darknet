...
def detect_image(net, meta, image, thresh=.5, hier_thresh=.5, nms=.45, debug=True):
    # import cv2
    ...
    dets = get_network_boxes(net, image.shape[1], image.shape[0], thresh, hier_thresh, None, 0, pnum, letter_box) # OpenCV
...
def performDetect(imagePath="data/test.jpg", thresh=0.25, configPath="yolov4-tiny.cfg", weightPath="yolov4-tiny.weights", metaPath="data/obj.data", showImage=False, makeImageOnly=False, initOnly=False):
    """
...
class Darknet:
    ...
    def detect_img_path(self, image_path, thresh=0.25):
        return detect(netMain, metaMain, image_path.encode("ascii"), thresh)

    def detect_cv_img(self, image, thresh=0.25):
        return detect_image(netMain, metaMain, image, thresh)
...
if __name__ == "__main__":
    ...
    (cfg, weights, meta) = YoloModelPath(model_path).get_cfg_weight_meta()
    detector = Darknet(cfg, weights, meta)

    image = cv2.imread("data/test.jpg")
    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    print(detector.get_classes())
    dets = detector.detect_cv_img(image)
    print(dets)
